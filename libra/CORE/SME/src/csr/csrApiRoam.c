...
eHalStatus csrRoamClose(tpAniSirGlobal pMac, tANI_U32 sessionId);
...
eHalStatus csrSendMBSetContextReqMsg( tpAniSirGlobal pMac, tANI_U32 sessionId, 
           ...
           tANI_U8 *pKeyRsc );
static void csrRoamIssueReassociate( tpAniSirGlobal pMac, tANI_U32 sessionId, 
...
#ifdef WLAN_SOFTAP_FEATURE
eHalStatus csrSetRegInfo(tHalHandle hHal,  char *apCntryCode)
{
    eHalStatus status = eHAL_STATUS_SUCCESS;
    ...
    status = csrGetRegulatoryDomainForCountry(pMac, (tANI_U8*)apCntryCode, &regId);
    if (status != eHAL_STATUS_SUCCESS)
    {
        smsLog( pMac, LOGE, FL("  fail to get regId for country Code %s\n"), apCntryCode );
		return status;
    }
    status = halPhySetRegDomain(hHal, regId);
    if (status != eHAL_STATUS_SUCCESS)
    {
        smsLog( pMac, LOGE, FL("  fail to get regId for country Code %s\n"), apCntryCode );
		return status;
...
eHalStatus csrClose(tpAniSirGlobal pMac)
{
    ...
    {
         csrStop(pMac, sessionId);
         csrRoamClose(pMac, sessionId);
     }
    csrScanClose(pMac);
#ifdef FEATURE_WLAN_GEN6_ROAMING
...
eHalStatus csrStop(tpAniSirGlobal pMac, tANI_U32 sessionId)
{
    ...
    if (!CSR_IS_SESSION_VALID( pMac, sessionId )) {
        smsLog(pMac, LOGE, FL("Session %d is not Active, Nothing to do\n"), sessionId);
        return status;
    }
                             ...
                             void *pUsrContext, void *pfnSapEventCallback )
{
    ...
    status = csrSendMBGetWPSPBCSessions( pMac, sessionId, bssId, pUsrContext, pfnSapEventCallback);            
        ...
        case eCsrReassocSuccess:
            ...
            */
            ...
            if( pSirBssDesc )
            {
                ...
                assocInfo.sessionId = (tANI_U8)sessionId;
...
#ifdef WLAN_SOFTAP_FEATURE
                roamInfo.staId =  pSmeStartBssRsp->staId;
#endif
...
eHalStatus csrRoamProcessDisassocDeauth( tpAniSirGlobal pMac, tSmeCmd *pCommand, tANI_BOOLEAN fDisassoc, tANI_BOOLEAN fMICFailure )
{
    ...
    else if ( csrIsConnStateInfra( pMac, sessionId ) )
    {
        ...
        if( fDisassoc )
        {
            status = csrRoamIssueDisassociate( pMac, sessionId, eCSR_ROAM_SUBSTATE_DISASSOC_FORCED, fMICFailure );
	    }
		else
		{
...
static void csrRoamJoinRspProcessor( tpAniSirGlobal pMac, tSirSmeJoinRsp *pSmeJoinRsp )
{
   ...
   else
   {
        ...
        if (pCommand)
        {
            if( CSR_IS_WDS( &pCommand->u.roamCmd.roamProfile ) )
            {
                csrRoamComplete( pMac, eCsrNothingToJoin, NULL );
            }
            else
...
static void csrRoamIssueReassociate( tpAniSirGlobal pMac, tANI_U32 sessionId, tSirBssDescription *pSirBssDesc, 
                              tDot11fBeaconIEs *pIes, tCsrRoamProfile *pProfile)
{
    ...
    smsLog(pMac, LOGE, FL("  calling csrRoamIssueReassociate\n"));
    // attempt to Join this BSS...
    csrSendSmeReassocReqMsg( pMac, sessionId, pSirBssDesc, pIes, pProfile );
}
                                            ...
                                            tANI_U8 *pBuf, tANI_U8 uapsdMask)
{
    ...
    *pBuf++ = (tANI_U8)FALSE;  //tAniTitanCBNeighborInfo->cbBssFoundSecUp
	...
	else 
	{
        ...
        {
        // powerCap
        pBuf += sizeof(tSirMacPowerCapInfo);
        // supportedChannels
        pBuf += sizeof(tSirSupChnl);
    }
    }
                                    ...
                                    tANI_U8 *pAssocStasBuf )
{
    ...
    do
    {
        ...
        pMsg->length = pal_cpu_to_be16(sizeof(tANI_U32 ) + (pBuf - wTmpBuf));//msg_header + msg
                                    ...
                                    tCsrRoamStartBssParams *pParam, tSirBssDescription *pBssDesc )
{
	...
	nwType = (tSirNwType)pal_cpu_to_be32(pParam->sirNwType);
	   ...
	   *pBuf = pParam->operationalRateSet.numRates; //tSirMacRateSet->numRates
        ...
        msgLen = sizeof(tANI_U32 ) + (pBuf - wTmpBuf); //msg_header + msg
        pMsg->length = pal_cpu_to_be16(msgLen);
