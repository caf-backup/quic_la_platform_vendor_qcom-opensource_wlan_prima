...
ifneq (, $(filter msm7627_surf msm7627_ffa msm7627_7x_ffa msm7627_7x_surf msm7630_surf msm7630_fusion, $(TARGET_PRODUCT)))
...
WLAN_RF_OUT           := $(WLAN_OUT)/CORE/HDD/src/libra.ko
WLAN_RF_FTM_OUT       := $(WLAN_OUT)/ftm/CORE/HDD/src/libra_ftm.ko
...
$(WLAN_RF_OUT): $(KERNEL_OUT) $(KERNEL_CONFIG) $(TARGET_PREBUILT_KERNEL)
	$(MAKE) -C kernel M=../$(WLAN)/CORE/HDD/src O=../$(KERNEL_OUT) ARCH=arm CROSS_COMPILE=arm-eabi- WLAN=../$(WLAN)

$(WLAN_RF_FTM_OUT): $(KERNEL_OUT) $(KERNEL_CONFIG) $(TARGET_PREBUILT_KERNEL) $(WLAN_RF_OUT)
	$(MAKE) -C kernel M=../$(WLAN)/ftm/CORE/HDD/src O=../$(KERNEL_OUT) ARCH=arm CROSS_COMPILE=arm-eabi- BUILD_FTM_DRIVER=1 WLAN=../$(WLAN)
	
$(WLAN_PRODUCT_OUT): $(ACP_BINARY_OUT) $(WLAN_RF_OUT) $(MAKE_MODULES_FOLDER)
	$(ACP) -f $(WLAN_RF_OUT) $(WLAN_PRODUCT_OUT)

$(WLAN_FTM_PRODUCT_OUT): $(ACP_BINARY_OUT) $(WLAN_PRODUCT_OUT) $(WLAN_RF_FTM_OUT) $(MAKE_MODULES_FOLDER)
	$(ACP) -f $(WLAN_RF_FTM_OUT) $(WLAN_FTM_PRODUCT_OUT)
	
$(WLAN_LIBRA_SDIOIF_OUT): $(ACP_BINARY_OUT) $(WLAN_RF_OUT) $(MAKE_MODULES_FOLDER) $(TARGET_PREBUILT_KERNEL)
	$(ACP) -f $(KERNEL_OUT)/drivers/net/wireless/libra/librasdioif.ko $(WLAN_LIBRA_SDIOIF_OUT)

all: $(WLAN_RF_OUT) $(WLAN_RF_FTM_OUT) $(WLAN_PRODUCT_OUT) $(WLAN_FTM_PRODUCT_OUT) $(WLAN_LIBRA_SDIOIF_OUT) $(MAKE_SYMBOLIC_LINK)
