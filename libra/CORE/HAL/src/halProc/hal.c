...
tANI_U32 uTxBknCnt = 0;
tANI_U32 uTxPreBknCnt = 0;
#endif
...
#if WLAN_SOFTAP_FW_BEACON_TX_PRNT_LOG
            uTxBknCnt++;
            HALLOGE( halLog(pMac, LOGE, FL("[SoftApFwBcnTx] SIR_HAL_SEND_BEACON_REQ-uTxBknCnt %d \n"), uTxBknCnt));
#endif
            break;
...
#ifdef FIXME_GEN6
    // This code needs to be fixed once halPhy Calibration is in
    // place with multi-BSS. We need to check rxpMode on a per BSS
    // basis.
#endif
...
static
eHalStatus halHandleInfraStaKeepAliveConfig(tpAniSirGlobal pMac, tANI_U32 cfgId)
{
    tANI_U32 val;
    eHalStatus status = eHAL_STATUS_SUCCESS;
