...
//Dtim Interval Threshold
#define WLAN_SOFTAP_DTIM_INTERVAL_THRESHOLD  50000
    ...
    tpAddStaParams  param, eAniBoolean isFreeable)
{
    ...
    if(param->supportedRates.opRateMode != eSTA_11b &&
...
#endif
    {
        ...
        if(param->staType == STA_ENTRY_SELF && bssIdx < 
            ...
            /* note: in STA and IBSS mode, the sequences are different from the upper layer 
               ...
               */
            // Since staType is STA_ENTRY_SELF, staIdx should be the selfidx
...
#ifdef WLAN_SOFTAP_FEATURE
    {
        tStaInfo staInfo;
        tANI_U16 macAddrHi;
        tANI_U32 offset = pMac->hal.memMap.staTable_offset + (staIdx * pMac->hal.memMap.staTable_size);

        HALLOGE(halLog(pMac, LOGE, FL("Adding station %d info to FW - bss %d, systemRole=%d, staType=%d\n"), staIdx, bssIdx, systemRole, param->staType));

        vos_mem_set( (void *)&staInfo, sizeof(tStaInfo), 0);
        staInfo.valid = 1;
        staInfo.bssIdx = bssIdx;        
        staInfo.isQosSta     = param->wmmEnabled;
        staInfo.uapsdMask    = param->uAPSD;
        staInfo.maxSPLen     = param->maxSPLen;

        vos_mem_copy( (void*)&staInfo.staMacAddrLo, (void*) &param->staMac[0], sizeof(tANI_U32));
        vos_mem_copy( (void*)&macAddrHi, (void*) &param->staMac[4], sizeof(tANI_U16));
        staInfo.staMacAddrHi = macAddrHi;

        HALLOGE(halLog(pMac, LOGE, FL("WmmEn=%d, Uapsd=%d, maxSP=%d\n"), param->wmmEnabled, param->uAPSD, param->maxSPLen));

        // Initialize the station related info
        if(systemRole == eSYSTEM_AP_ROLE) {
            if(param->staType == STA_ENTRY_SELF) {
                HALLOGE(halLog(pMac, LOGE, FL("Adding self station %d for softAP, bss %d\n"), staIdx, bssIdx));
                staInfo.staType = STA_TYPE_AP_SELF;
            } else if (param->staType == STA_ENTRY_PEER) {
                staInfo.staType = STA_TYPE_STA_PEER;
                staInfo.listenIntv  =  param->listenInterval;
                HALLOGE(halLog(pMac, LOGE, FL("[SoftAP]Adding peer station %d for softAP, bss %d ListIntv %d \n"), staIdx, bssIdx, staInfo.listenIntv));
            }
        } else if(systemRole == eSYSTEM_STA_ROLE) {
            if(param->staType == STA_ENTRY_SELF) {
                HALLOGE(halLog(pMac, LOGE, FL("Adding self station %d for Infra STA, bss %d\n"), staIdx, bssIdx));
                staInfo.staType = STA_TYPE_STA_SELF;
            } else if (param->staType == STA_ENTRY_PEER) {
                staInfo.staType = STA_TYPE_AP_PEER;
                HALLOGE(halLog(pMac, LOGE, FL("[SoftAP]Adding peer station %d for AP, bss %d \n"), staIdx, bssIdx));
            }
        }

        staInfo.aid   = param->assocId;
        // Update the STA info in the shared memory
        halWriteDeviceMemory(pMac, offset, (void*)&staInfo, pMac->hal.memMap.staTable_size );
        halFW_AddStaMsg(pMac, staIdx);
    }
#endif    
    ...
    tpDeleteStaParams   pDelStaReq)
{
    ...
    if( STA_ENTRY_SELF != staType )
    {
        ...
        else
        {
            ...
            /* 
               beacon_tsf_tim_extract_ctrl
               beacon_addr2_low_field
               beacon_addr2_high_field
               beacon_addr3_low_field
               beacon_addr3_high_field
               Above registers are set to 0 with the below call. As a result these 
               registers are not programmed as they should be 
               and basically you have wrong addr2/addr3 and tsf_tim_extract_ctrl 
               register set. The symptom that was observed is that our TBTT is no 
               more in sync with the peer group. And as a result we keep on transmitting 
               beacons even after receiving one. */
    ...
    {
        tStaInfo  staInfo;
        tANI_U32 offset = pMac->hal.memMap.staTable_offset + (pDelStaReq->staIdx * pMac->hal.memMap.staTable_size);
        ...
        // Update the STA info in the shared memory
        halWriteDeviceMemory(pMac, offset, (void*)&staInfo, pMac->hal.memMap.staTable_size );
        halFW_DelStaMsg(pMac, pDelStaReq->staIdx);
    }
#endif    
        ...
        tANI_U32 status, tANI_U16 dialog_token)
{
              ...
              (param->staContext.staType == STA_ENTRY_PEER))))
    {
        ...
        // Enable ssid based filtering in the rxp for IBSS.
            ...
            (BssSystemRole == eSYSTEM_BTAMP_AP_ROLE))
    {
        ...
        HALLOGE( halLog(pMac, LOGE, FL("Soft AP... Adding AP role\n")));
        param->staContext.staType = STA_ENTRY_SELF;
        param->staContext.staIdx  = bssStaIdx;
        palCopyMemory(pMac->hHdd, &(param->staContext.staMac), param->bssId, sizeof(tSirMacAddr));
        halMsg_AddSta(pMac,  dialog_token, &(param->staContext), eANI_BOOLEAN_FALSE);
            ...
            (BssSystemRole == eSYSTEM_AP_ROLE))
    {
        ...
        /* Always write beacon template on BSS INDEX = 0. This is a limitation
           ...
           */ 
        halTpe_SetBeaconTemplate(pMac, 0, param->bssId);
...

    {
        tBssInfo  bssInfo;
        tANI_U16  bssIdHi;
        tANI_U32 offset = pMac->hal.memMap.bssTable_offset + ( bssIdx * pMac->hal.memMap.bssTable_size);

        vos_mem_set( (void *)&bssInfo, sizeof(tBssInfo), 0);
        // BSS valid bit
        bssInfo.valid       = 1;
        // Station ID associated with the BSS
        bssInfo.staId       = bssStaIdx;      
        bssInfo.bcastStaId  = bcastStaIdx;
        if (BssSystemRole == eSYSTEM_AP_ROLE) {
            bssInfo.bssType     = BSS_TYPE_AP_SELF;
        } else if (BssSystemRole == eSYSTEM_STA_ROLE) {
            bssInfo.bssType     = BSS_TYPE_AP_PEER;
        } else if (BssSystemRole == eSYSTEM_STA_IN_IBSS_ROLE) {
            bssInfo.bssType = BSS_TYPE_IBSS;
        }

        // If QoS is enabled
        //bssInfo.qosAp     = 1;
        bssInfo.beaconIntv  = param->beaconInterval;
        bssInfo.dtimPeriod      = param->dtimPeriod;
        bssInfo.dtimIntervalTh  = WLAN_SOFTAP_DTIM_INTERVAL_THRESHOLD;

        vos_mem_copy( (void*)&bssInfo.bssidLo, (void*) &param->bssId[0], sizeof(tANI_U32));
        vos_mem_copy( (void*)&bssIdHi, (void*) &param->bssId[4], sizeof(tANI_U16));
        bssInfo.bssidHi = bssIdHi;

        //probe Response Max retries
        bssInfo.ucMaxProbeRespRetryLimit =  param->ucMaxProbeRespRetryLimit;
        //To Enable Disable Proxy Probe Resp
        ...
        // Send Message to FW
        halFW_AddBssMsg( pMac,  bssIdx );

    }
#endif


    //TODO : conditional compilation to be removed once we are in system_ap_role for softap
    if (((BssSystemRole == eSYSTEM_STA_IN_IBSS_ROLE) ||
                ((BssSystemRole == eSYSTEM_BTAMP_STA_ROLE) && (savedBTStatype == STA_ENTRY_SELF))) ||
            (BssSystemRole == eSYSTEM_BTAMP_AP_ROLE))
    {

#if WLAN_SOFTAP_FW_BEACON_TRANSMIT
        HALLOGE( halLog(pMac, LOGE, FL("[SoftApFwBcnTx]-halMsg_AddBssPostSetChan-InitPreBeaconTmr skip... \n")));
#else
    ...
    tpDeleteBssParams   pDelBssReq)
{
            ...
            (BssSystemRole == eSYSTEM_AP_ROLE))
    {
...
#ifdef WLAN_SOFTAP_FEATURE
        {
            ...
            HALLOGE( halLog(pMac, LOGE, FL("[SoftApFwBcnTx]-halMsg_DelBss->H2FDelBssMsg bssIndx[%d]...\n"),pDelBssReq->bssIdx ));
#endif
        }
#endif
...
#if WLAN_SOFTAP_FW_BEACON_TX_PRNT_LOG
        HALLOGE( halLog(pMac, LOGE, FL("[SoftApFwBcnTx-ERROR]-halMsg_DelBss->Host Disable Timer skip...\n") ));     
#endif
...
#ifdef WLAN_SOFTAP_FEATURE  
    HALLOGE( halLog(pMac, LOGE, FL("[SoftApFwBcnTx]-halMsg_HandleInitScan Disable Beacon TX skipped...\n")));
#else
    // Make sure to Disable beacon transmission. Hence enable flag is set to FALSE.
    halMTU_EnableDisableBssidTBTTBeaconTransmission(pMac,
            pMac->hal.halMac.beaconInterval, FALSE);
    HALLOGE( halLog(pMac, LOGE, FL("[NON-SOFTAP]-halMsg_HandleInitScan Disable Beacon TX ..\n")));
#endif
...
eHalStatus halMsg_setPromiscMode(tpAniSirGlobal pMac)
{
...

    {
         ...
         */
#ifdef FIXME_GEN6
        // Code not used currently for Gen6.
#endif
    ...
    tpSendbeaconParams  msg)
{
    ...
    if (halTable_GetBssIndex(pMac, msg->bssId, &bssIndex) != eHAL_STATUS_DUPLICATE_BSSID){
        ...
        /* Always write beacon template on BSS INDEX = 0. This is a limitation
           ...
           In Volcance this has to be reverted to use proper BSS INDEX 
           */ 
...
#ifdef WLAN_SOFTAP_FEATURE
    pBssTable[bssIndex].bcnTimIeOffset = msg->TimIeOffset - sizeof(((tAniBeaconStruct*) msg->beacon )->beaconLength);
#ifdef WLAN_SOFTAP_FW_BEACON_TX_PRNT_LOG
    HALLOGE( halLog(pMac, LOGE, FL("[SoftApFwBcnTx]-halMsg_SendBeacon-bcnTimIeOffset:[%d]...\n"), pBssTable[bssIndex].bcnTimIeOffset ));
#endif
#endif
...
#if WLAN_SOFTAP_FW_BEACON_TRANSMIT
    if( pBssTable[bssIndex].bcnSendIndCnt ==  0) {
        halFW_SendBeaconMsg(pMac, bssIndex );
        HALLOGE( halLog(pMac, LOGE, FL("[SoftApFwBcnTx]-halMsg_SendBeacon-bssIndx[%d] bcnSndIndCnt[%d]...\n"),  bssIndex , pBssTable[bssIndex].bcnSendIndCnt ));
        pBssTable[bssIndex].bcnSendIndCnt++;
    } else {
    ...
    halMTU_EnableDisableBssidTBTTBeaconTransmission(pMac,
            pMac->hal.halMac.beaconInterval, TRUE);
    HALLOGE( halLog(pMac, LOGE, FL("[NON-SoftAp]-halMsg_SendBeacon To EnaBcnTx...\n")));
#endif
...
eHalStatus halPrepareForBmpsExit(tpAniSirGlobal pMac)
{
    //halRxp_setRxpFilterMode(pMac, eRXP_POST_ASSOC_MODE, NULL);
    // The global filter is cleared, only the BSS specific
    // rxp Mode is used. If its in post assoc mode then
    // it will go back to post assoc mode.
    halRxp_setSystemRxpFilterMode(pMac, eRXP_IDLE_MODE, eHAL_USE_BSS_RXP);
...
eHalStatus halMsg_sendBmpsInit(tpAniSirGlobal pMac)
{
    return eHAL_STATUS_SUCCESS;
}

eHalStatus halPrepareForBmpsEntry(tpAniSirGlobal pMac)
{

    //halRxp_setRxpFilterMode(pMac, eRXP_POWER_SAVE_MODE, NULL);
    //halRxp_setRxpFilterMode(pMac, eRXP_POWER_SAVE_MODE);
    // Set the rxp mode = global rxp mode & Bss rxp mode.
    halRxp_setSystemRxpFilterMode(pMac, eRXP_POWER_SAVE_MODE,
...
eHalStatus halMsg_exitPowerSave(tpAniSirGlobal pMac)
{
#ifdef FIXME_GEN6 //Code here is not valid for GEN6

    if (tx_timer_activate(&pMac->hal.PSRspTimer) != TX_SUCCESS)
    {
    HALLOG1( halLog(pMac, LOG1, FL("Unable to activate SoftMAC response message timer\n")));
    return eHAL_STATUS_FAILURE;
    }

        HALLOGW( halLog(pMac, LOGW, FL("HAL: Request bit successfully Asserted while exiting power save\n")));
#endif
    return eHAL_STATUS_SUCCESS;
}
...
halMsg_BeaconPre( tpAniSirGlobal pMac )
{
    ...
    *
    * This code holds good for VOLANCE
    */
#ifdef FIXME_VOLANCE
    ...
    for (staIdx = 0; staIdx < pMac->hal.memMap.maxStations; staIdx++) {
        ...
        if (halTable_GetBssIndexForSta(pMac, &bssIdx, staIdx) != eHAL_STATUS_SUCCESS)
            return; /* eHAL_STATUS_FAILURE; - void function*/
