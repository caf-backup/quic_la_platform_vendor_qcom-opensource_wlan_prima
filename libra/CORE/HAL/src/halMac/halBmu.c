...
#endif
    // NOTE: Set this bit as we have seen when querying BTQM queue, BTQM at times 
    // does not get queue access grant and as a result of which the access_done bit 
    // in transmit_queue_access_control_4 register is never set when read command is
    // issued. Setting this bit avoids running into such a scenario. 
    value = QWLAN_BMU_BMU_BACKUP_CONTROL_CFG_CLEAR_DA_ON_FINDING_EMPTY_QUEUE_ENABLE_MASK;
    halWriteRegister(pMac, QWLAN_BMU_BMU_BACKUP_CONTROL_REG, value);
