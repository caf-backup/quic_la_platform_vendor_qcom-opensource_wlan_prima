...
void halLog_dumpWMMProfile(tpAniSirGlobal pMac)
{
    ...
    HALLOGW( halLog(pMac, LOGW, FL(" |--------------------------------------------------------------------------|\n")));
    HALLOGW( halLog(pMac, LOGW, FL(" |                             TID -> AC MAPPING                            |\n")));
    HALLOGW( halLog(pMac, LOGW, FL(" |--------------------------------------------------------------------------|\n")));
    for (staid=0;  staid < pMac->hal.halMac.maxSta; staid++)
    {
        if ( t[staid].valid != 0 )
        {
            halTable_GetStaTidToAcMap(pMac, staid, &tidToAcMap);
            HALLOGW( halLog( pMac, LOGW, FL(" |  STAID:%-4d                       |        TidToAcMap  : 0x%x      |\n"),  staid, tidToAcMap ));
            HALLOGW( halLog(pMac, LOGW, FL(" |--------------------------------------------------------------------------|\n")));
            HALLOGW( halLog(pMac, LOGW, FL(" | TID          :      0      1      2      3      4      5      6      7   |\n")));
            HALLOGW( halLog(pMac, LOGW, FL(" |--------------------------------------------------------------------------|\n")));
            HALLOGW( halLog(pMac, LOGW, FL(" | AC           :     %s     %s     %s     %s     %s     %s     %s     %s   |\n"),
                               AC2STR[((tidToAcMap & (3<<0)) >> 0)], AC2STR[((tidToAcMap & (3<<2)) >> 2)],
                               AC2STR[((tidToAcMap & (3<<4)) >> 4)], AC2STR[((tidToAcMap & (3<<6)) >> 6)],
                               AC2STR[((tidToAcMap & (3<<8)) >> 8)], AC2STR[((tidToAcMap & (3<<10)) >> 10)],
                               AC2STR[((tidToAcMap & (3<<12)) >> 12)], AC2STR[((tidToAcMap & (3<<14)) >> 14)]));
            HALLOGW( halLog(pMac, LOGW, FL("-|--------------------------------------------------------------------------|-\n")));
        }
    }
...
#ifdef WLAN_SOFTAP_FEATURE
static char *
dump_hal_send_sap_cmd( tpAniSirGlobal pMac, tANI_U32 arg1, tANI_U32 arg2, tANI_U32 arg3, tANI_U32 arg4, char *p)
{
    Qwlanfw_SapUtilStruct sapUtil;

    sapUtil.cmd     = arg1;
    sapUtil.staIdx  = arg2;
    sapUtil.queueId = arg3;

    halFW_SendMsg(pMac, HAL_MODULE_ID_SOFT_AP,
            QWLANFW_HOST2FW_SAP_UTIL_CMD, dialogToken, sizeof(Qwlanfw_SapUtilStruct), &sapUtil, FALSE, NULL);
    return p;
}
...
dump_hal_set_ap_link_monitor( tpAniSirGlobal pMac, tANI_U32 enable, tANI_U32 arg2, tANI_U32 arg3, tANI_U32 arg4, char *p)
{
    ...
    HALLOGW( halLog(pMac, LOGE, FL("link monitoring enabled = %d\n"), !pFwConfig->fDisLinkMonitor));
    halFW_UpdateSystemConfig(pMac, pMac->hal.FwParam.fwSysConfigAddr, (tANI_U8 *)pFwConfig, sizeof(Qwlanfw_SysCfgType));
    return p;
}
...
dump_hal_set_ap_unknown_addr2_handling( tpAniSirGlobal pMac, tANI_U32 enable, tANI_U32 arg2, tANI_U32 arg3, tANI_U32 arg4, char *p)
{
    ...
    HALLOGW( halLog(pMac, LOGE, FL("unknown addr2 handling enabled = %d\n"), pFwConfig->fEnableFwUnknownAddr2Handling));
    halFW_UpdateSystemConfig(pMac, pMac->hal.FwParam.fwSysConfigAddr, (tANI_U8 *)pFwConfig, sizeof(Qwlanfw_SysCfgType));
    return p;
}
...
dump_sendFcFrameToFw(tpAniSirGlobal pMac, tANI_U32 staIdx, tANI_U32 memTh, tANI_U32 fcConfig, tANI_U32 arg4, char* p)
{
    ...
    fcTxBd.fcSTAThreshEnabledMask = (1 << staIdx); 
...
static char *
dump_hal_set_sap_fw_routing( tpAniSirGlobal pMac, tANI_U32 arg1, tANI_U32 arg2, tANI_U32 arg3, tANI_U32 arg4, char *p)
{
    if(arg1) {
...
dump_hal_set_sap_fw_timBasedDisassoc( tpAniSirGlobal pMac, tANI_U32 arg1, tANI_U32 arg2, tANI_U32 arg3, tANI_U32 arg4, char *p)
{
    Qwlanfw_SysCfgType *pFwConfig = (Qwlanfw_SysCfgType *)pMac->hal.FwParam.pFwConfig;
    (void) arg2; (void) arg3; (void) arg4;
    pFwConfig->bTimBasedDisAssocEna = (tANI_U8)arg1;
    HALLOGE( halLog(pMac, LOGE, FL("[SoftAp]TIMBasedDisAssocFlag[%d]\n"), pFwConfig->bTimBasedDisAssocEna ));
    halFW_UpdateSystemConfig(pMac, pMac->hal.FwParam.fwSysConfigAddr, (tANI_U8 *)pFwConfig, sizeof(Qwlanfw_SysCfgType));
    return p;
}
#endif
