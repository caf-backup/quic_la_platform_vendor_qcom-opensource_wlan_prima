...
eHalStatus halMemoryMap_Start(tHalHandle hHal, void *arg)
{
    ...
    pMac->hal.memMap.btqmTxQueue_offset =
      (((pMac->hal.memMap.rpePartialBitmap_offset+pMac->hal.memMap.rpePartialBitmap_size)+7)/8) * 8;
...
#ifdef WLAN_SOFTAP_FEATURE
    pMac->hal.memMap.bssTable_offset = pMac->hal.memMap.beaconTemplate_offset+(BEACON_TEMPLATE_SIZE * pMac->hal.memMap.maxBssids);
    pMac->hal.memMap.bssTable_size   = BSS_INFO_SIZE;
    

    pMac->hal.memMap.staTable_offset = pMac->hal.memMap.bssTable_offset + (pMac->hal.memMap.bssTable_size * pMac->hal.memMap.maxBssids);
    pMac->hal.memMap.staTable_size   = STA_INFO_SIZE;

#if WLAN_SOFTAP_FW_PROCESS_PROBE_REQ_FEATURE
    pMac->hal.memMap.probeRespTemplate_offset  = 
	        pMac->hal.memMap.staTable_offset + (pMac->hal.memMap.staTable_size * pMac->hal.memMap.maxStations);
    pMac->hal.memMap.probeRespTemplate_size	  = WLAN_SOFTAP_FW_PROBE_RSP_TEMPLATE_SIZE;
    pMac->hal.memMap.aduUmaStaDesc_offset =
	        pMac->hal.memMap.probeRespTemplate_offset + (pMac->hal.memMap.probeRespTemplate_size * pMac->hal.memMap.maxBssids);
#else 
    pMac->hal.memMap.aduUmaStaDesc_offset =
        pMac->hal.memMap.staTable_offset+ (pMac->hal.memMap.staTable_size * pMac->hal.memMap.maxStations);
#endif //--->#if WLAN_SOFTAP_FW_PROCESS_PROBE_REQ_FEATURE

#else  //--> #ifdef WLAN_SOFTAP_FEATURE
    pMac->hal.memMap.aduUmaStaDesc_offset =
        pMac->hal.memMap.beaconTemplate_offset+
