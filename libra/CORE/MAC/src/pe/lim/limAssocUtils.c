                          ...
                          tANI_U16 protStatusCode,tpPESession psessionEntry)
{
   ...
   chanNum = psessionEntry->currentOperChannel;
    ...
    limSetChannel(pMac, psessionEntry->limCurrentTitanHtCaps, chanNum, psessionEntry->peSessionId);
...
void limHandleCnfWaitTimeout(tpAniSirGlobal pMac, tANI_U16 staId)
{
    ...
    //to get the MlME state based on sessionId, use the below seesionEntry
    //priority - HIGH
    tpPESession psessionEntry;
    ...
    switch (pStaDs->mlmStaContext.mlmState) {
        case eLIM_MLM_WT_ASSOC_CNF_STATE:
            ...
            if ( (pMac->lim.gLimSystemRole == eLIM_AP_ROLE)|| (psessionEntry->limSystemRole == eLIM_BT_AMP_AP_ROLE) )
            {
                limRejectAssociation(
                            pMac,
                            pStaDs->staAddr,
...
limDeleteDphHashEntry(tpAniSirGlobal pMac, tSirMacAddr staAddr, tANI_U16 staId,tpPESession psessionEntry)
{
    ...
    if (pStaDs != NULL)
    {
        ...
        limUtilCountStaDel(pMac, pStaDs, psessionEntry->limSystemRole);
