...
/* timeout handler function which signals the event sem and 
 * wakes up the thread which was sleeping on the timer 
 */
...
VOS_STATUS vos_event_set ( vos_event_t* event ) 
{
   ...
   set_bit(VOS_EVENT_SET, &event->event_flags);
  
   if (event->process != NULL)
   {
         wake_up_process(event->process);
   }
...
{
   VOS_STATUS uResult = VOS_STATUS_SUCCESS;
   
   clear_bit(VOS_EVENT_SET, &event->event_flags);
   return( uResult );
}
...
VOS_STATUS vos_wait_single_event ( vos_event_t* event, v_U32_t timeout)
{
   ...
   if (timeout)
   {
      timer_event.event_flags = 0;
      timer_event.process = current;
      init_timer(&my_timer);
      my_timer.expires = jiffies + msecs_to_jiffies(timeout);
      my_timer.data = (unsigned long) &timer_event;
      my_timer.function = process_timeout;
      add_timer(&my_timer);
   }
      ...
      for (;;)
      {
          /* relinquish processor until event occurs */
      	  if (timeout && test_bit(VOS_EVENT_SET, &timer_event.event_flags))
          {
	      /* reset event now that it is set */
	      clear_bit(VOS_EVENT_SET, &timer_event.event_flags);
      	      timer_event.process = NULL;
              timeout_event = 1;
       	      event->process = NULL;
              set_current_state(TASK_RUNNING);
              break;
          }
	  if (!test_bit(VOS_EVENT_SET, &event->event_flags))
	  {
              schedule();
              set_current_state(TASK_INTERRUPTIBLE);
	  }
      ...
      if (timeout)
         del_singleshot_timer_sync(&my_timer);
   ...
   v_U32_t timeout, v_U8_t *pEventIndex )
{
   ...
   if ((events == NULL) || (pEventIndex == NULL) || (timeout < 0))
      return VOS_STATUS_E_FAULT;
   ...
   {
   // check if cookie is same as that of initialized event
      if (events[i]->cookie != LINUX_EVENT_COOKIE )      
      {
         return VOS_STATUS_E_INVAL;
      }
      events[i]->process = current;
   }
   ...
   allow_signal(SIGKILL);  // we die if we get a SIGKILL signal
   ...
   for (;;)
   {
      ...
      for (i=0; i< numEvents; i++)
      {
           if (test_bit(VOS_EVENT_SET, &events[i]->event_flags)) /* found a matching event */
            {
		 int j;
	         /* reset event now that it is set */
   		
	         clear_bit(VOS_EVENT_SET, &events[i]->event_flags);
                 *pEventIndex = i;
     		 found = 1;
                  
   	         for (j=0; j<numEvents; j++)
   	         {
       		   events[j]->process = NULL;
   	         }
