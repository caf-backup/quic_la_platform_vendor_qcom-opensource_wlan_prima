...
static t2Decimal absReportedWithPrecision(tANI_U8 pdet)
{
    ...
    tANI_S16 reported2Decimal = (pdet + yIntercept) * 100/slope;
...
{
	int chan, point;
	for(chan=0;chan < numTpcCalFreqs; chan++)
	{
		printk(KERN_EMERG "------------\n");
		printk(KERN_EMERG "freq: %d\n", table[chan].freq);
		printk(KERN_EMERG "numTpcCalPoints: %d\n", table[chan].empirical[0].numTpcCalPoints);
		for(point = 0; point < 4;point++)
		{
			printk(KERN_EMERG "temperatureAdc: %d\n", table[chan].empirical[0].chain[point].temperatureAdc);
			printk(KERN_EMERG "txGain: %d\n", table[chan].empirical[0].chain[point].txGain);
			printk(KERN_EMERG "pwrDetAdc: %d\n", table[chan].empirical[0].chain[point].pwrDetAdc);
			printk(KERN_EMERG "absPowerMeasured: %x\n", (int)(table[chan].empirical[0].chain[point].absPowerMeasured.measurement));
		}
	}
}
/*********************************/
    ...
    if (numTpcCalFreqs == 2)
    {
        ...
        for (chanIndex = 0; chanIndex < numTpcCalFreqs; chanIndex++)
        {
            ...
            {
                printk(KERN_EMERG "MAX_TPC_CAL_POINTS for chIdx: %d, txChain: %d, --> %d\n", (int)chanIndex, (int)txChain, (int)(table[chanIndex].empirical[txChain].numTpcCalPoints));
...
eQWPttStatus pttGrabRam(tpAniSirGlobal pMac, tANI_U32 startSample, tANI_U32 numSamples, tGrabRamSample *grabRam)
{
    ...
    {
        printk(KERN_EMERG "pttGrabRam param check failed\n");
        return (FAILURE);
    }
        ...
        {
            printk(KERN_EMERG "asicGrabAdcSamples failed\n");
            return (FAILURE);
        }
