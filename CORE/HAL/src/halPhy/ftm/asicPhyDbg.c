...
*/
    ...
    SET_PHY_REG(pMac->hHdd, QWLAN_PHYDBG_PRBS_REG, 0x10);                               //seed value
    SET_PHY_REG(pMac->hHdd, QWLAN_PHYDBG_PRBS_MS_REG, 0x5432);
...
static eHalStatus DoGrabRamCapture(tpAniSirGlobal pMac, tANI_U32 rxChain);
/* not used
...
#define INTERLEAVE_OFFSET_0  (128 * 1024)
#ifdef SPICA_FPGA
...
eHalStatus asicGrabAdcSamples(tpAniSirGlobal pMac, tANI_U32 startSample, tANI_U32 numSamples, tGrabRamSample *sampleBuffer)
{
    ...
    if (startSample == 0)
    {
        ...
        {
            if (DoGrabRamCapture(pMac, rxChain) != eHAL_STATUS_SUCCESS)
        {
            return(eHAL_STATUS_FAILURE);
        }
    }
    ...
    pAdcSamples1 = pMac->ptt.pADCCaptureCache + GRAB_RAM_DBLOCK_SIZE; //for rxChain 1
...
static eHalStatus DoGrabRamCapture(tpAniSirGlobal pMac, tANI_U32 rxChain)
{
    ...
    tANI_U32 xBarSel[PHY_MAX_RX_CHAINS] = {QWLAN_PHYDBG_CAPT_CFG_XBARSEL_EC32_ADC_CH0, QWLAN_PHYDBG_CAPT_CFG_XBARSEL_EC32_ADC_CH1};
    tANI_U8 *startPtr;
...
*/
    ...
    //self._csr.phydbg.cfgmode(crccapture0=0, sc0=0, ec0=0, cont0=0, dbgmem_sel0=int(use_dbg_mem))
    if ((retVal = rdModWrAsicField(pMac, QWLAN_PHYDBG_CFGMODE_REG, QWLAN_PHYDBG_CFGMODE_CRCCAPTURE0_MASK, QWLAN_PHYDBG_CFGMODE_CRCCAPTURE0_OFFSET, 0)) != eHAL_STATUS_SUCCESS) { return(retVal);}
    if ((retVal = rdModWrAsicField(pMac, QWLAN_PHYDBG_CFGMODE_REG, QWLAN_PHYDBG_CFGMODE_SC0_MASK, QWLAN_PHYDBG_CFGMODE_SC0_OFFSET, 0)) != eHAL_STATUS_SUCCESS) { return(retVal);}
    if ((retVal = rdModWrAsicField(pMac, QWLAN_PHYDBG_CFGMODE_REG, QWLAN_PHYDBG_CFGMODE_EC0_MASK, QWLAN_PHYDBG_CFGMODE_EC0_OFFSET, 0)) != eHAL_STATUS_SUCCESS) { return(retVal);}
    if ((retVal = rdModWrAsicField(pMac, QWLAN_PHYDBG_CFGMODE_REG, QWLAN_PHYDBG_CFGMODE_CONT0_MASK, QWLAN_PHYDBG_CFGMODE_CONT0_OFFSET, 0)) != eHAL_STATUS_SUCCESS) { return(retVal);}
    if ((retVal = rdModWrAsicField(pMac, QWLAN_PHYDBG_CFGMODE_REG, QWLAN_PHYDBG_CFGMODE_DBGMEM_SEL0_MASK, QWLAN_PHYDBG_CFGMODE_DBGMEM_SEL0_OFFSET, 0)) != eHAL_STATUS_SUCCESS) { return(retVal);}

    //c32_adc_ch0, c32_adc_ch1: ID=27
...
*/
/*
    // Explicitely disenable clcok to PHYDBG module
        if ((retVal = rdModWrAsicField(pMac, QWLAN_RXCLKCTRL_APB_BLOCK_CLK_EN_REG, QWLAN_RXCLKCTRL_APB_BLOCK_CLK_EN_PHYDBG_MASK, QWLAN_RXCLKCTRL_APB_BLOCK_CLK_EN_PHYDBG_OFFSET, 0)) != eHAL_STATUS_SUCCESS)
        {
            return(retVal);
        }
...
eHalStatus log_grab_ram(tpAniSirGlobal pMac, tANI_U32 startSample, tANI_U32 numSamples)
{
    if (((startSample + numSamples) <= GRAB_RAM_DBLOCK_SIZE) && (numSamples <= MAX_REQUESTED_GRAB_RAM_SAMPLES))
    {
        ...
        if (eHAL_STATUS_SUCCESS == palAllocateMemory(pMac->hHdd, (void **)&buffer, numSamples * sizeof(tGrabRamSample)))
        {
            ...
            {
                phyLog(pMac, LOGE, "%04d: \t%-05d \t%-05d \t%-05d \t%-05d\n",
                       i,
