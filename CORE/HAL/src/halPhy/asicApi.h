...
{                                                                                       \
    const char *regStr = GetLibraRegName(addr);                                          \
    if (regStr)                                                                         \
    {                                                                                   \
        tANI_U32 _val = 0;                                                              \
        GET_PHY_REG(pMac->hHdd, addr, &_val);                                           \
        phyLog(pMac, LOGE, "(%08X):%60s = 0x%08X\n", addr, regStr, _val);               \
    }                                                                                   \
    else                                                                                \
    {                                                                                   \
        phyLog(pMac, LOGE, "0x%08X not found in register list\n", addr);                \
    }                                                                                   \
}
...
eHalStatus asicSetAGCCrossbar(tpAniSirGlobal pMac, eAGCRxChainMask rxMaskEnabled);
