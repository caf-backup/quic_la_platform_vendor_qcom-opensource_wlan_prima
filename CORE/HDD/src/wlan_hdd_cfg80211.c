                                      ...
                                      )
{
    ...
    status = sme_RoamSetKey( pAdapter->hHal, &setKey, &roamId );
                                      ...
                                      )
{
    ...
    sme_RoamGetConnectProfile(hHal, &roamProfile);
...
{  
    ...
    status = sme_ScanRequest( pAdapter->hHal, &scanRequest, &scanId, 
        ...
        const u8 *ssid, size_t ssid_len, const u8 *bssid)
{
    ...
    if (pRoamProfile) 
    {
        ...
        status = sme_RoamConnect( pAdapter->hHal, pRoamProfile, NULL, &roamId);
                                         ...
                                         )
{
    ...
    if (NULL != pRoamProfile)
    {
        ...
        if (pAdapter->conn_info.connState == eConnectionState_Associated)
        {
            ...
            /*issue disconnect*/
            status = sme_RoamDisconnect( pAdapter->hHal, reasonCode);
                                         ...
                                         )
{
    ...
    init_completion(&pAdapter->disconnect_comp_var);
    vosStatus = sme_RoamDisconnect( pAdapter->hHal, eCSR_DISCONNECT_REASON_IBSS_LEAVE);
