                                      ...
                                      struct cfg80211_ap_settings *params)
{
       ...
       )
    {
        ...
        wlan_hdd_cfg80211_set_channel(wiphy, dev, params->chandef.chan,
                                         ...
                                         struct station_parameters *params)
{
    ...
    txrx_pdev_ctx = vos_get_context(VOS_MODULE_ID_TXRX,
				    pHddCtx->pvosContext);
...

    {
                ...
                (pAdapter->device_mode == WLAN_HDD_P2P_CLIENT))
     {
	    ...
	    wdi_in_peer_state_update(txrx_pdev_ctx,
            ...
            VOS_TRACE( VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO,
                    "%s: TXRX Moving to Authenticated state", __func__);
	}
     }
    EXIT();
    return status;
...
{   
    ...
    while (pScanResult)
    {
	    ...
	    cfg80211_put_bss(pAdapter->wdev.wiphy, bss_status);
#else
                    ...
                    (WLAN_HDD_P2P_GO == pAdapter->device_mode))
            {
                for (staId = 0; staId < WLAN_MAX_STA_COUNT; staId++)
                {
                    if ((pAdapter->aStaInfo[staId].isUsed) && 
                            (WLAN_STA_CONNECTED == pAdapter->aStaInfo[staId].sta_state))
                    {
...
#if defined (WLAN_FEATURE_11W) && (LINUX_VERSION_CODE > KERNEL_VERSION(3,8,0))
                                       ...
                                       )
{
    ...
    {
        u8 channelNum;
        if (IEEE80211_BAND_5GHZ == channel->band)
        {
            hddLog(VOS_TRACE_LEVEL_ERROR, 
