                                       ...
                                       tSirBssDescription *pBssDesc )
{
	...
	hdd_station_ctx_t *pHddStaCtx = WLAN_HDD_GET_STATION_CTX_PTR(pAdapter);
	hdd_context_t *pHddCtx = WLAN_HDD_GET_CTX(pAdapter);
	v_VOID_t *txrx_pdev_ctx = vos_get_context(VOS_MODULE_ID_TXRX,
						  pHddCtx->pvosContext);


   /* TODO: Regisrter the rx callback with txrx, notify
    * of authenticated state to txrx, if needed.
    */
	param.qos_capable =  hdd_wmm_is_active(pAdapter);
	wdi_in_peer_update(pAdapter->txrx_vdev_ctx,
			   (u_int8_t *) pPeerMacAddress->bytes, &param,
			   ol_txrx_peer_update_qos_capable);

	VOS_TRACE( VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO_MED,
		  "ULA auth StaId= %d.  Changing TXRX state to CONNECTED at Join time",
		  pHddStaCtx->conn_info.staId[ 0 ] );

	wdi_in_peer_state_update(txrx_pdev_ctx,
				 (u_int8_t *) pPeerMacAddress->bytes,
   ...
   staDesc.ucInitState = pRoamInfo->fAuthRequired ?
      WLAN_STA_CONNECTED : WLAN_STA_AUTHENTICATED;
   ...
   if ( !pRoamInfo->fAuthRequired )
   {
      ...
      vosStatus = wlan_change_sta_state( pHddCtx->pvosContext, staDesc.ucSTAId, 
                                                    ...
                                                    eCsrRoamResult roamResult )
{
    ...
    if ( eCSR_ROAM_RESULT_ASSOCIATED == roamResult )
    {
	    ...
	    wdi_in_peer_state_update(txrx_pdev_ctx,
				     (u_int8_t *) pRoamInfo->peerMac,
				     ol_txrx_peer_state_conn);
	    pHddStaCtx->conn_info.uIsAuthenticated = VOS_FALSE;

            VOS_TRACE(VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_INFO,
		      "%s: staId: %d Changing TXRX state to CONNECTED",
                                    ...
                                    tANI_U8 *peerMac, tANI_U16 staId, tANI_U8 ucastSig)
{
    ...
    staDesc.ucInitState = WLAN_STA_AUTHENTICATED ;
                                ...
                                eRoamCmdStatus roamStatus, eCsrRoamResult roamResult )
{
    ...
    switch( roamStatus )
    {
        case eCSR_ROAM_SESSION_OPENED:
            if(pAdapter != NULL)
            {
                pAdapter->txrx_vdev_ctx = pRoamInfo->txrx_vdev_hdl;
