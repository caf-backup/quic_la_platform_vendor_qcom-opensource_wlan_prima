...
int
htt_h2t_mgmt_tx(struct htt_pdev_t   *pdev,
                A_UINT32             paddr,
                adf_nbuf_t           mgmt_frm,
                A_UINT32             frag_len,
                A_UINT16             vdev_id,
...

{
    ...
    pkt->msdu_id = HTT_TX_COMPL_INV_MSDU_ID;
    pkt->pdev_ctxt = NULL; /* not used during send-done callback */
    msg = htt_tx_mgmt_desc_alloc(pdev, &desc_id, mgmt_frm);
    if (!msg) {
        htt_htc_pkt_free(pdev, pkt);
        return -1; /* failure */
    }
    /* set the length of the message */
    //adf_nbuf_put_tail(msg, sizeof(struct htt_mgmt_tx_desc_t));
...
#else
    adf_os_mem_copy(tx_mgmt_desc->hdr, hdr, HTT_MGMT_FRM_HDR_DOWNLOAD_LEN);
#endif
    SET_HTC_PACKET_INFO_TX(
        &pkt->htc_pkt,
        NULL,
