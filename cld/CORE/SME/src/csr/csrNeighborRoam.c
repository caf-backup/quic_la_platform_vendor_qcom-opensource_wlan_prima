...
VOS_STATUS csrNeighborRoamSetLookupRssiThreshold(tpAniSirGlobal pMac, v_U8_t neighborLookupRssiThreshold)
{
    ...
    if (eCSR_NEIGHBOR_ROAM_STATE_CONNECTED == pNeighborRoamInfo->neighborRoamState)
    {
        ...
        /* Register Neighbor Lookup threshold callback with TL for DOWN event only */
	/* TODO: Fix it when TXRX is used */
#if 0
        vosStatus = WLANTL_RegRSSIIndicationCB(pMac->roam.gVosContext, (v_S7_t)pNeighborRoamInfo->currentNeighborLookupThreshold * (-1),
                                                WLAN_HO_THRESHOLD_DOWN,
                                                csrNeighborRoamNeighborLookupDOWNCallback,
                                                VOS_MODULE_ID_SME, pMac);
#endif
        if (!VOS_IS_STATUS_SUCCESS(vosStatus))
                               ...
                               v_S7_t   avgRssi)
{
    ...
    vosStatus = wlan_txrx_deregister_rssi_indcb(pMac->roam.gVosContext, (v_S7_t)pNeighborRoamInfo->cfgParams.neighborReassocThreshold * (-1),
                                                        WLAN_HO_THRESHOLD_DOWN, 
    ...
    vosStatus = wlan_txrx_deregister_rssi_indcb(pMac->roam.gVosContext,
                        (v_S7_t)NEIGHBOR_ROAM_LOOKUP_UP_THRESHOLD * (-1),
                        WLAN_HO_THRESHOLD_UP,
...
eHalStatus csrNeighborRoamPreauthRspHandler(tpAniSirGlobal pMac, tSirRetStatus limStatus)
{
    ...
    else
    {
        ...
        vosStatus = wlan_txrx_register_rssi_indcb(pMac->roam.gVosContext,
                            (v_S7_t)NEIGHBOR_ROAM_LOOKUP_UP_THRESHOLD * (-1),
                            WLAN_HO_THRESHOLD_UP,
                            csrNeighborRoamNeighborLookupUPCallback,
                            VOS_MODULE_ID_SME, pMac);
...
//                        palTimerStop(pMac->hHdd, pNeighborRoamInfo->neighborScanTimer);
        ...
        if (eHAL_STATUS_SUCCESS != hstatus)
            ...
            vosStatus = wlan_txrx_register_rssi_indcb(pMac->roam.gVosContext, (v_S7_t)pNeighborRoamInfo->cfgParams.neighborReassocThreshold * (-1),
                                            WLAN_HO_THRESHOLD_DOWN, 
                                            csrNeighborRoamReassocIndCallback,
                                            VOS_MODULE_ID_SME, pMac);
...
VOS_STATUS  csrNeighborRoamNeighborLookupUpEvent(tpAniSirGlobal pMac)
{
    ...
    vosStatus = wlan_txrx_register_rssi_indcb(pMac->roam.gVosContext, (v_S7_t)pNeighborRoamInfo->currentNeighborLookupThreshold * (-1),
                                    WLAN_HO_THRESHOLD_DOWN, 
                                    csrNeighborRoamNeighborLookupDOWNCallback, 
                                    VOS_MODULE_ID_SME, pMac);
#ifdef FEATURE_WLAN_LFR
...
VOS_STATUS  csrNeighborRoamNeighborLookupDownEvent(tpAniSirGlobal pMac)
{
    ...
    switch (pNeighborRoamInfo->neighborRoamState)
    {
        case eCSR_NEIGHBOR_ROAM_STATE_CONNECTED:
            ...
            vosStatus = wlan_txrx_deregister_rssi_indcb(pMac->roam.gVosContext, (v_S7_t)pNeighborRoamInfo->currentNeighborLookupThreshold * (-1),
                                            WLAN_HO_THRESHOLD_DOWN, 
                                            csrNeighborRoamNeighborLookupDOWNCallback, 
                                            VOS_MODULE_ID_SME);
                                                                               ...
                                                                               v_S7_t avgRssi)
{
    ...
    VOS_ASSERT(WLAN_HO_THRESHOLD_UP == rssiNotification);
                                                                               ...
                                                                               v_S7_t avgRssi)
{
    ...
    VOS_ASSERT(WLAN_HO_THRESHOLD_DOWN == rssiNotification);
    ...
    switch (pNeighborRoamInfo->neighborRoamState)
    {
        ...
        case eCSR_NEIGHBOR_ROAM_STATE_INIT:
            ...
            if ( init_ft_flag == TRUE )
            {
                ...
                NEIGHBOR_ROAM_DEBUG(pMac, LOG2, FL("Registering neighbor lookup DOWN event with TL, RSSI = %d"), pNeighborRoamInfo->currentNeighborLookupThreshold);
                /* Register Neighbor Lookup threshold callback with TL for DOWN event only */
                vstatus = wlan_txrx_register_rssi_indcb(pMac->roam.gVosContext, (v_S7_t)pNeighborRoamInfo->currentNeighborLookupThreshold * (-1),
                                            WLAN_HO_THRESHOLD_DOWN, 
                                            csrNeighborRoamNeighborLookupDOWNCallback, 
                                            VOS_MODULE_ID_SME, pMac);
#ifdef FEATURE_WLAN_LFR
