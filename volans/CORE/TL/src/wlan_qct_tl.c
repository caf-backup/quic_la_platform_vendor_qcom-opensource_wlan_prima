...
#define WLANTL_BD_PDU_RESERVE_THRESHOLD           120
...
)
{
  ...
  if (( 0 == pTLCb->usPendingTxCompleteCount ) &&
      ( pTLCb->uResCount <= WLANTL_BD_PDU_RESERVE_THRESHOLD ))
  {
     VOS_TRACE( VOS_MODULE_ID_TL, VOS_TRACE_LEVEL_INFO_HIGH,
...
)
{
  ...
  ucUnicastBroadcastType  = (v_U16_t)WLANHAL_RX_BD_GET_UC_BC(aucBDHeader);
#endif
  if(0 != ucMPDUHLen)
  {
  ...
  if(VOS_TRUE == pTLCb->atlSTAClients[ucSTAId].ucIsReplayCheckValid)
  {
      ...
      if ( 0 != ucAsf )
      {
          ...
          if( 0 != ucEsf )
          {
              ...
              ullcurrentReplayCounter = WLANTL_GetReplayCounterFromRxBD(aucBDHeader);
