...
static int driver_mode = eDRIVER_TYPE_PRODUCTION;
 ...
 *-------------------------------------------------------------------------*/
extern int isWDresetInProgress(int driver_mode_flag);
...
int hdd_wlan_sdio_probe(struct sdio_func *sdio_func_dev )
{
    ...
    * cfg80211 initialization and registration....
    */
   wiphy = wlan_hdd_cfg80211_init(&sdio_func_dev->dev, sizeof(hdd_context_t)) ;
   ...
   //Save the driver_mode to the adapter for future usage
   if(eDRIVER_TYPE_MFG == driver_mode)
   {
       pHddCtx->driver_type = eDRIVER_TYPE_MFG ;
   }
...
err_balstop:
   if(eDRIVER_TYPE_PRODUCTION == pHddCtx->driver_type)
   {
...
static void __exit hdd_module_exit(void)
{
   ...
   {
      while(isWDresetInProgress(pHddCtx->driver_type) || vos_is_logp_in_progress(VOS_MODULE_ID_HDD,NULL))
      {
...
VOS_STATUS wlan_hdd_restart_driver(hdd_context_t *pHddCtx)
{
   ...
   if(atomic_read(&pHddCtx->isRestartInProgress) == 1)
   {
...
module_param(driver_mode, int, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
MODULE_PARM_DESC(driver_mode, "i");
