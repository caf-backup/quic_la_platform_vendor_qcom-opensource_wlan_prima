    ...
    void        *arg)
{
    ...
    tANI_U32       regVal = 0;
#ifdef WLAN_HAL_VOLANS
    /*In Volans release L replay check is always done at host*/
    tANI_U32       value = 0;
#endif
    (void) arg;
...
#ifdef WLAN_HAL_VOLANS
   /*In Volans release L replay check will always be done at host*/
   /*Set 12th bit of DPU control register, such that DPU cal fill replay counter in 6 bytes PMI command*/
   halReadRegister(pMac, QWLAN_DPU_DPU_CONTROL_REG, &value);
   value |= QWLAN_DPU_DPU_CONTROL_INCL_PN_IN_BD_EN_MASK;		
   halWriteRegister(pMac, QWLAN_DPU_DPU_CONTROL_REG, value);
#endif
   /*Programming element ID field in MMIE as 4C as required by 802.11w spec*/
   halReadRegister(pMac, QWLAN_DPU_DPU_BIP_MMIE_REG, &regVal);
   regVal |= QWLAN_DPU_DPU_BIP_ELEMENT_ID;
   halWriteRegister(pMac, QWLAN_DPU_DPU_BIP_MMIE_REG, regVal);
    return eHAL_STATUS_SUCCESS;
}
