...
{
    tpTpeRateTable pRateTableEntry = (tpTpeRateTable)(&tpeSramRateTable[rateIdx]);

    halRateTable[rateIdx].tpeRateTableDynamic.cntrlRateIdx = pRateTableEntry->cntrlRateIdx;
    halRateTable[rateIdx].tpeRateTableDynamic.cntrlRspTxPwr = pRateTableEntry->cntrlRspTxPwr;
    halRateTable[rateIdx].tpeRateTableDynamic.protCntrlRateIdx = pRateTableEntry->protCntrlRateIdx;    
    halRateTable[rateIdx].tpeRateTableDynamic.rspRateIdx = pRateTableEntry->rspRateIdx;
}
#endif
...
eHalStatus halRate_Start(tHalHandle hHal, void *arg)
{
    ...
    for (i=0; i < MAX_LIBRA_TX_RATE_NUM; i++) {
        halRate_UpdateRateTableDynamicData(pMac, i);        
#else
    ...
    status = halRate_UpdateTpeRateTable(pMac, MIN_LIBRA_RATE_NUM, MAX_LIBRA_TX_RATE_NUM);
#else
...
eHalStatus halRate_UpdateTpeRateTable(tpAniSirGlobal pMac, tANI_U16 startRate, tANI_U16 endRate)
{
    ...
    eHalStatus status = eHAL_STATUS_FAILURE;
#ifdef WLAN_HAL_VOLANS
    tpTpeSramRateTableDynamic pTpeRateTableEntry;
#else
...
#ifdef WLAN_HAL_VOLANS
        pTpeRateTableEntry = (tpTpeSramRateTableDynamic)&halRateTable[i].tpeRateTableDynamic;

        if ((status = halTpe_RateTableWrite(pMac, i, pTpeRateTableEntry->data
                        )) != eHAL_STATUS_SUCCESS) {
            return status;
        }
#else
...
#ifdef WLAN_HAL_VOLANS
    tANI_U32 index;
    tpTpeSramRateTableDynamic pTpeRate = NULL;
    ...
    for (index = MIN_LIBRA_RATE_NUM; index < MAX_LIBRA_TX_RATE_NUM; index++) {
        if (rateIdx == (tTpeRateIdx)pHalRate->tpeRateTableDynamic.rspRateIdx) {
            ...
            if(pHalRate->tpeRateTableDynamic.cntrlRspTxPwr != txPower) {
                pHalRate->tpeRateTableDynamic.cntrlRspTxPwr = txPower;
                
                if (updateTpeHw) {
                    pTpeRate = (tpTpeSramRateTableDynamic)&halRateTable[index].tpeRateTableDynamic;
                    // Update the txpower for the rate index in the TPE rate table
                    halTpe_RateTableWrite(pMac, index, pTpeRate->data);
                }
            }
        }
        pHalRate++;
...
eHalStatus halRate_sendResponseRateTable(tpAniSirGlobal pMac)
{
    ...
    for (i = 0; i < MAX_LIBRA_TX_RATE_NUM; i++) {
        halRateTable[i].tpeRateTableDynamic.rspRateIdx = rspRateIndexTable[i];
        halRateTable[i].tpeRateTableDynamic.cntrlRateIdx = rspRateIndexTable[i];
...
eHalStatus halRate_BckupTpeRateTable(tpAniSirGlobal pMac, tANI_U32 *pAddr)
{
    ...
    for (i = 0; i < MAX_LIBRA_TX_RATE_NUM; i++) {
        pTpeRateTableEntry = (tpTpeSramRateTableDynamic)&pRateTable[i].tpeRateTableDynamic;
        ...
        value = ((i << QWLAN_TPE_RATE_TABLE_SRAM_CONTROL_RATE_SRAM_XACT_ADDR_OFFSET) |
                 (pTpeRateTableEntry->data << QWLAN_TPE_RATE_TABLE_SRAM_CONTROL_RATE_SRAM_XACT_DATA_OFFSET) |
