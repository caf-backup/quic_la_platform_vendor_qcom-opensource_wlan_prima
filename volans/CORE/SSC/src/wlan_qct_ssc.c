...
---------------------------------------------------------------------------*/
/* This size is reduced from 0x700 to 0x400 to avoid reading unnecessarily large
data from SDIO. This size is used only while reading the first CMD53 after RX 
interrupt and sizes for the subsequent CMD53s are derived from the already read CMD53
*/
#define WLANSSC_MAXPKTSIZE 0x400
...
#ifdef DUMP_SSC_STATS
/**
 @brief WLANSSC_DumpStats is used to dump all the statistics in the SSC
 control block
 ...
 @return
*/
static v_VOID_t WLANSSC_DumpStats
(
  WLANSSC_ControlBlockType *pControlBlock
)
{
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  WLANSSC_MSG_INFO2( "Int = %d, NumRxCmd53 = %d, uNumRxFrames = %d\n", pControlBlock->sStatsInfo.uNumInterrupts, pControlBlock->sStatsInfo.uNumRxCmd53, pControlBlock->sStatsInfo.uNumRxFrames);
  WLANSSC_MSG_INFO2( "NumRxPadding = %d, NumRxPartial = %d, NumRxRemainder = %d\n", pControlBlock->sStatsInfo.uNumRxPadding, pControlBlock->sStatsInfo.uNumRxPartial, pControlBlock->sStatsInfo.uNumRxRemainder);
  WLANSSC_MSG_INFO2( "NumTxCmd53 = %d, NumTxFrames = %d\n", pControlBlock->sStatsInfo.uNumTxCmd53, pControlBlock->sStatsInfo.uNumTxFrames, 0);
} /* WLANSSC_ResetStats() */
#endif /* DUMP_SSC_STATS */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (vos_is_logp_in_progress(VOS_MODULE_ID_SSC, NULL)) 
  {
     VOS_TRACE( VOS_MODULE_ID_SSC, VOS_TRACE_LEVEL_ERROR, "%s: LOGP in progress, aborting this command \n", __FUNCTION__);
     return VOS_STATUS_SUCCESS;
  }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
...
#ifdef DUMP_SSC_STATS
   if ((pControlBlock->sStatsInfo.uNumTxFrames & 0x1FFF) == 0 )
       WLANSSC_DumpStats(pControlBlock);
#endif /* DUMP_SSC_STATS */
  ...
  /* The below sleep was causing a huge throughput drop when aggregation enabled.
...
#ifdef DUMP_SSC_STATS
        if ((pControlBlock->sStatsInfo.uNumRxFrames & 0x1FFF) == 0 )
            WLANSSC_DumpStats(pControlBlock);
#endif /* DUMP_SSC_STATS */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /* RX FIFO                                                               */
  sCmd53Request.address = QWLAN_SIF_RXF_ACC0_PTR_REG;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  SSCLOGE(VOS_TRACE( VOS_MODULE_ID_SSC, VOS_TRACE_LEVEL_FATAL, "Fatal error interrupt received!"));
...
)
{
  ...
  /* At this point the STOP bit in the DXE descriptor for the Rx DXE channel is set.
     ...
     read and another corresponding to next DXE descriptor read with STOP bit). So, 
     read data coresponding to 2 max-sized packets (Libra Specific)
   */
  
  /* However, the scenario for Volans is little different. In Volans, the SIF RxFIFO 
   * depth is large enough (512 bytes) to accommodate both Rx channels, DXE_RX and 
   * DXE_RX_HI. Hence, there will be totally 4 packets (two for each channel as mentioned above) 
