...
#include <asm/div64.h>
#define __do_div64(x,y,z)   do{\
             x = (y);\
             do_div(x, (z));\
         }while(0)
#else
#define __do_div64(x, y,z)  (x = (y)/(z))
#endif
// --------------------------------------------------------------------
/**
 * pmmInitialize
...
void pmmInitBmpsResponseHandler(tpAniSirGlobal pMac, eHalStatus rspStatus)
{
    ...
    */
	...
	if(rspStatus == eHAL_STATUS_SUCCESS)
    {
		...
		limSendSmeRsp(pMac, eWNI_PMC_ENTER_BMPS_RSP,  retStatus);
    }
    else
    {
...
void pmmExitBmpsRequestHandler(tpAniSirGlobal pMac, tpExitBmpsInfo pExitBmpsInfo)
{
    ...
    limDiagEventReport(pMac, WLAN_PE_DIAG_EXIT_BMPS_REQ_EVENT, NULL, 0, (tANI_U16)pExitBmpsInfo->exitBmpsReason);
    ...
    if(limIsSystemInScanState(pMac))
    {
        ...
        limSendSmeRsp(pMac, eWNI_PMC_EXIT_BMPS_RSP, respStatus);
		return;
	}
...
void pmmInitBmpsPwrSave(tpAniSirGlobal pMac)
{
...
#ifdef FEATURE_WLAN_DIAG_SUPPORT 
    limDiagEventReport(pMac, WLAN_PE_DIAG_ENTER_BMPS_REQ_EVENT, NULL, 0, 0);
#endif //FEATURE_WLAN_DIAG_SUPPORT

    if (pMac->lim.gLimSystemRole != eLIM_STA_ROLE)
    {
        PELOGE(pmmLog(pMac, LOGE, 
            FL("pmmBmps: Invalid PowerSave Init msg in role %d\n"),
            pMac->lim.gLimSystemRole);)

        respStatus = eSIR_SME_INVALID_STA_ROLE;
        pmmBmpsUpdateReqInInvalidRoleCnt(pMac);
        goto failure;
    }
	...
	//Do not continue if the device is not associated
    if(eLIM_SME_IDLE_STATE == pMac->lim.gLimSmeState)
    {
        PELOGE(pmmLog(pMac, LOGE, 
            FL("pmmBmps: Station Not associated yet, BMPS request rejected\n"));)
        respStatus = eSIR_SME_STA_NOT_ASSOCIATED;
        goto failure;
    }
    ...
    if((retStatus = pmmSendInitPowerSaveMsg(pMac)) != eSIR_SUCCESS)
    {
        PELOGE(pmmLog(pMac, LOGE, 
    ...
    {
        //wait for an explicit response  from  softmac
       PELOG1(pmmLog(pMac, LOG1,
            FL("pmmBmps: waiting for ENTER_BMPS response from softmac\n"));)
    }
    return;
...
failure:
    ...
    pMac->pmm.gPmmState =origState;
    limSendSmeRsp(pMac, eWNI_PMC_ENTER_BMPS_RSP, respStatus);
    return;
}
...
tSirRetStatus pmmSendChangePowerSaveMsg(tpAniSirGlobal pMac)
{
    ...
    msgQ.type = SIR_HAL_EXIT_BMPS_REQ;
    msgQ.reserved = 0;
     ...
     * get a SIR_HAL_EXIT_BMPS_RSP from HAL.
     */
    SET_LIM_PROCESS_DEFD_MESGS(pMac, false);
    MTRACE(macTraceMsgTx(pMac, 0, msgQ.type));
    retStatus = halPostMsgApi( pMac, &msgQ);
    if( eSIR_SUCCESS != retStatus )
    {
        PELOGE(pmmLog( pMac, LOGE, FL("Sending SIR_HAL_EXIT_BMPS_REQ failed, reason=%X \n"), retStatus );)
        palFreeMemory(pMac->hHdd, (tANI_U8*)pExitBmpsParams);
        return retStatus;
    }

    PELOG1(pmmLog(pMac, LOG1, FL("SIR_HAL_EXIT_BMPS_REQ has been successfully sent to HAL\n"));)
    return retStatus;
}
...
tSirRetStatus  pmmSendInitPowerSaveMsg(tpAniSirGlobal pMac)
{
    ...
    if(pMac->lim.gCurrentBssBeaconCnt == 0)
    {
    ...
    pBmpsParams->tbtt = pMac->lim.gLastBeaconTimeStamp;
    pBmpsParams->dtimCount = pMac->lim.gLastBeaconDtimCount;
    pBmpsParams->dtimPeriod = pMac->lim.gLastBeaconDtimPeriod;
    pBmpsParams->bssIdx = pMac->lim.gLimBssIdx;
    ...
    PELOGW(pmmLog( pMac, LOGW,
        FL( "pmmBmps: Sending SIR_HAL_ENTER_BMPS_REQ" ));)

    /* we need to defer any incoming messages until we get a
     * SIR_HAL_ENTER_BMPS_RSP from HAL.
     */
    SET_LIM_PROCESS_DEFD_MESGS(pMac, false);
    ...
    if( eSIR_SUCCESS != (retCode = halPostMsgApi( pMac, &msgQ )))
    {
        ...
        PELOGE(pmmLog( pMac, LOGE,
            FL("Posting SIR_HAL_ENTER_BMPS_REQ to HAL failed, reason=%X\n"),
            retCode );)
    }
...
tSirRetStatus pmmSendPowerSaveCfg(tpAniSirGlobal pMac, tpSirPowerSaveCfg pUpdatedPwrSaveCfg)
{
    ...
    {
        pmmLog(pMac, LOGE,
            FL("pmmCfg: Power Save Configuration received in invalid role %d"),
            pMac->lim.gLimSystemRole);
        retCode = eSIR_PMM_INVALID_ROLE;
        goto returnFailure;
    }
    ...
    msgQ.type = SIR_HAL_PWR_SAVE_CFG;
    msgQ.reserved = 0;
    msgQ.bodyptr = pUpdatedPwrSaveCfg;
    msgQ.bodyval = 0;

    PELOG1(pmmLog( pMac, LOG1, FL( "pmmBmps: Sending SIR_HAL_PWR_SAVE_CFG to HAL"));)
    MTRACE(macTraceMsgTx(pMac, 0, msgQ.type));
    if( eSIR_SUCCESS != (retCode = halPostMsgApi( pMac, &msgQ )))
    {
        pmmLog( pMac, LOGP,
            FL("Posting SIR_HAL_PWR_SAVE_CFG to HAL failed, reason=%X"),
            retCode );
        goto returnFailure;
    }
    return retCode;
...
void pmmMissedBeaconHandler(tpAniSirGlobal pMac)
{
    ...
    PELOGE(pmmLog(pMac, LOGE, FL("The device woke up due to MISSED BEACON \n"));)

    /* Proceed only if HeartBeat timer is created */
    if((pMac->lim.limTimers.gLimHeartBeatTimer.pMac) &&	(pMac->lim.gLimTimersCreated))
    {
		...
		PELOG1(pmmLog(pMac, LOG1,
				...
				(pMac->lim.gLimTimersCreated))
			{
        		...
        		if (wlan_cfgGetInt(pMac, WNI_CFG_BEACON_INTERVAL,
        	    	      &beaconInterval) != eSIR_SUCCESS)
	    		{
    	    	PELOG1(pmmLog(pMac, LOG1,
						FL("pmmBmps: Could not retrieve BEACON_INTERVAL value\n"));)
    			}
...
/*
    ...
    if (pMac->pmm.BmpsWakeupTimeStamp){
        __do_div64( TimeAwake, (pMac->pmm.BmpsSleepTimeStamp - pMac->pmm.BmpsWakeupTimeStamp), 10);
    }else
    ...
    __do_div64(pMac->pmm.BmpsavgTimeAwake, ((pMac->pmm.BmpsavgTimeAwake * pMac->pmm.BmpscntSleep) + TimeAwake ), (pMac->pmm.BmpscntSleep + 1));
...
/*
        ...
        __do_div64(  SleepTime , (pMac->pmm.BmpsWakeupTimeStamp - pMac->pmm.BmpsSleepTimeStamp) ,10);
        ...
        __do_div64(pMac->pmm.BmpsavgSleepTime , ((pMac->pmm.BmpsavgSleepTime * pMac->pmm.BmpscntAwake) + SleepTime ) , (pMac->pmm.BmpscntAwake + 1) );
...
void pmmEnterImpsRequestHandler (tpAniSirGlobal pMac)
{
...
#ifdef FEATURE_WLAN_DIAG_SUPPORT 
    limDiagEventReport(pMac, WLAN_PE_DIAG_ENTER_IMPS_REQ_EVENT, NULL, 0, 0);
#endif //FEATURE_WLAN_DIAG_SUPPORT

	// IMPS is not applicable in STA mode
	if(pMac->lim.gLimSystemRole != eLIM_STA_ROLE)
	{
		PELOGE(pmmLog(pMac, LOGE, 
               FL("pmmImps: ImpsSleepModeRequest received in invalid role %d\n"),
			   pMac->lim.gLimSystemRole);)

		resultCode = eSIR_SME_INVALID_STA_ROLE;
		goto failure;
	}
	...
	// Reject IMPS request if STA associated
	if(eLIM_SME_LINK_EST_STATE == pMac->lim.gLimSmeState)
	{
		// association exists with the peer device
		PELOGE(pmmLog(pMac, LOGE, 
           	FL("pmmImps: Association exists with peer, power save request rejected\n"));)
		resultCode = eSIR_SME_STA_ASSOCIATED;
		goto failure;
	}
...
void pmmEnterImpsResponseHandler (tpAniSirGlobal pMac, eHalStatus rspStatus)
{
    ...
    /* we need to process all the deferred messages enqueued since
     * the initiating the SIR_HAL_ENTER_IMPS_REQ.
     */
    SET_LIM_PROCESS_DEFD_MESGS(pMac, true);

    // reject IMPS request in AP mode
	if(pMac->lim.gLimSystemRole != eLIM_STA_ROLE)
	{
		PELOGE(pmmLog(pMac, LOGE, 
            FL("pmmImps: invalid msg in the role %d\n"),
			pMac->lim.gLimSystemRole);)
	...
	if(eHAL_STATUS_SUCCESS == rspStatus)
	{
		...
		PELOGW(pmmLog(pMac, LOGW,
            FL("pmmImps: Received successful SIR_HAL_ENTER_IMPS_RSP from HAL\n"));)
...
#ifdef FEATURE_WLAN_DIAG_SUPPORT 
    limDiagEventReport(pMac, WLAN_PE_DIAG_EXIT_IMPS_REQ_EVENT, NULL, 0, 0);
#endif //FEATURE_WLAN_DIAG_SUPPORT
...
void pmmExitImpsResponseHandler(tpAniSirGlobal pMac, eHalStatus rspStatus)
{
    ...
    /* we need to process all the deferred messages enqueued since
     * the initiating the SIR_HAL_EXIT_IMPS_REQ.
     */
    SET_LIM_PROCESS_DEFD_MESGS(pMac, true);
	...
	switch(rspStatus)
	{
		case eHAL_STATUS_SUCCESS:
		{
			...
			PELOGW(pmmLog(pMac, LOGW, 
					FL("pmmImps: Received SIR_HAL_EXIT_IMPS_RSP with Successful response from HAL\n"));)
		}
		break;
        ...
        default:
		{
	        resultCode = eSIR_SME_IMPS_REQ_FAILED;
			PELOGW(pmmLog(pMac, LOGW, 
					FL("pmmImps: Received SIR_HAL_EXIT_IMPS_RSP with Failure Status from HAL\n"));)
		}
			break;
 ...
 * SIR_HAL_ENTER_UAPSD_REQ to HAL.
 *
 * NOTE:
 *
 * @param       Global handle to MAC
...
void pmmEnterUapsdRequestHandler (tpAniSirGlobal pMac)
{
...
#ifdef FEATURE_WLAN_DIAG_SUPPORT 
    limDiagEventReport(pMac, WLAN_PE_DIAG_ENTER_UAPSD_REQ_EVENT, NULL, 0, 0);
#endif //FEATURE_WLAN_DIAG_SUPPORT
    ...
    PELOG1(pmmLog(pMac, LOG1, FL("pmmUapsd: Waiting for SIR_HAL_ENTER_UAPSD_RSP \n"));)
    return;

failure:
    pMac->pmm.gPmmState = origState;
    limSendSmeRsp(pMac, eWNI_PMC_ENTER_UAPSD_RSP, resultCode);
    return;
}
...
void pmmEnterUapsdResponseHandler(tpAniSirGlobal pMac, tpSirMsgQ limMsg)
{
    ...
    if(pMac->pmm.gPmmState != ePMM_STATE_UAPSD_WT_SLEEP_RSP)
    {
        PELOGE(pmmLog(pMac, LOGE,
            ...
            pMac->pmm.gPmmState);)
        limSendSmeRsp(pMac, eWNI_PMC_ENTER_UAPSD_RSP, eSIR_SME_INVALID_PMM_STATE);        
        return;
    }
...
#ifdef FEATURE_WLAN_DIAG_SUPPORT 
    limDiagEventReport(pMac, WLAN_PE_DIAG_EXIT_UAPSD_REQ_EVENT, NULL, 0, 0);
#endif //FEATURE_WLAN_DIAG_SUPPORT
...
void pmmExitUapsdResponseHandler(tpAniSirGlobal pMac, eHalStatus rspStatus)
{
    ...
    if (pMac->pmm.gPmmState != ePMM_STATE_UAPSD_WT_WAKEUP_RSP)
    {
        PELOGE(pmmLog(pMac, LOGE,
            ...
            pMac->pmm.gPmmState);)
        limSendSmeRsp(pMac, eWNI_PMC_EXIT_UAPSD_RSP, eSIR_SME_INVALID_PMM_STATE);
        return;
    }
    ...
    pMac->pmm.gPmmState = ePMM_STATE_BMPS_SLEEP;
    limSendSmeRsp(pMac, eWNI_PMC_EXIT_UAPSD_RSP, resultCode);
    return;
}
...
void pmmSendWowlAddBcastPtrn(tpAniSirGlobal pMac,  tpSirMsgQ pMsg)
{
    ...
    msgQ.type = SIR_HAL_WOWL_ADD_BCAST_PTRN;
    msgQ.reserved = 0;
    msgQ.bodyptr = pBcastPtrn;
    msgQ.bodyval = 0;

    pmmLog(pMac, LOG1, FL( "Sending SIR_HAL_WOWL_ADD_BCAST_PTRN to HAL"));
#ifdef FEATURE_WLAN_DIAG_SUPPORT 
    ...
    if( eSIR_SUCCESS != (retCode = halPostMsgApi( pMac, &msgQ )))
    {
        ...
        pmmLog( pMac, LOGP, FL("Posting SIR_HAL_WOWL_ADD_BCAST_PTRN failed, reason=%X"), retCode );
    }
    return;
}
...
void pmmSendWowlDelBcastPtrn(tpAniSirGlobal pMac, tpSirMsgQ pMsg)
{
    ...
    msgQ.type = SIR_HAL_WOWL_DEL_BCAST_PTRN;
    msgQ.reserved = 0;
    msgQ.bodyptr = pDeletePtrn;
    msgQ.bodyval = 0;

    pmmLog(pMac, LOG1, FL( "Sending SIR_HAL_WOWL_DEL_BCAST_PTRN"));
#ifdef FEATURE_WLAN_DIAG_SUPPORT 
    ...
    if( eSIR_SUCCESS != (retCode = halPostMsgApi( pMac, &msgQ )))
    {
        ...
        pmmLog( pMac, LOGP, FL("Posting SIR_HAL_WOWL_DEL_BCAST_PTRN failed, reason=%X"), retCode );
    }
    return;
}
...
\        SIR_HAL_WOWL_ENTER_REQ message to HAL.
\param   tpAniSirGlobal  pMac
\param   tpSirMsgQ       pMsg
\return  None
 ------------------------------------------------------------*/
void pmmEnterWowlRequestHandler(tpAniSirGlobal pMac, tpSirMsgQ pMsg)
{
    ...
    if (pMac->lim.gLimSmeState != eLIM_SME_LINK_EST_STATE)
    {
        ...
        limSendSmeRsp(pMac, eWNI_PMC_ENTER_WOWL_RSP, eSIR_SME_STA_NOT_ASSOCIATED);
        goto end;
    }
    ...
    if ((pMac->pmm.gPmmState != ePMM_STATE_BMPS_SLEEP) && (pMac->pmm.gPmmState != ePMM_STATE_WOWLAN))
    {
        ...
        limSendSmeRsp(pMac, eWNI_PMC_ENTER_WOWL_RSP, eSIR_SME_INVALID_PMM_STATE);
        goto end;
    }
    ...
    if( eSIR_SUCCESS != (retCode = pmmSendWowlEnterRequest( pMac, pHalWowlParams)))
    {
        ...
        limSendSmeRsp(pMac, eWNI_PMC_ENTER_WOWL_RSP, eSIR_SME_WOWL_ENTER_REQ_FAILED);
        goto end;
    }
    return;
...
\brief   LIM sends a SIR_HAL_WOWL_ENTER_REQ message to HAL with
\        the message structure pHalWowlParams.  HAL shall later
\        send a SIR_HAL_WOWL_ENTER_RSP with the same pointer
...
tSirRetStatus pmmSendWowlEnterRequest(tpAniSirGlobal pMac, tpSirHalWowlEnterParams pHalWowlParams)
{
    ...
    {
        pmmLog( pMac, LOGE, FL("Posting SIR_HAL_WOWL_ENTER_REQ failed, reason=%X"), retCode );
        return retCode;
    }
    return retCode;
}
...
\brief   LIM process the SIR_HAL_WOWL_ENTER_RSP message.
...
void pmmEnterWowlanResponseHandler(tpAniSirGlobal pMac, tpSirMsgQ limMsg)
{
     ...
     * since the initiating the SIR_HAL_WOWL_ENTER_REQ.
     */
    SET_LIM_PROCESS_DEFD_MESGS(pMac, true);
    ...
    if (NULL == pHalWowlMsg)
    {
        pmmLog(pMac, LOGE, FL("Recvd SIR_HAL_WOWL_ENTER_RSP with NULL msg "));
    ...
    limSendSmeRsp(pMac, eWNI_PMC_ENTER_WOWL_RSP, smeRspCode);
    return;
}
...
\        and sends SIR_HAL_WOWL_EXIT_REQ to HAL.
\param   tpAniSirGlobal  pMac
\return  None
 ------------------------------------------------------------*/
void pmmExitWowlanRequestHandler(tpAniSirGlobal pMac)
{
    ...
    {
        pmmLog(pMac, LOGE,
            FL("Fail to send SIR_HAL_WOWL_EXIT_REQ, reason code %d\n"),
            retStatus);
...
failure:
    limSendSmeRsp(pMac, eWNI_PMC_EXIT_WOWL_RSP, smeRspCode);
    return;
}

/** ---------------------------------------------------------
\fn      pmmSendExitWowlReq
\brief   This function sends the SIR_HAL_WOWL_EXIT_REQ
\        message to HAL.
\param   tpAniSirGlobal  pMac
\return  None
 ------------------------------------------------------------*/
tSirRetStatus  pmmSendExitWowlReq(tpAniSirGlobal pMac)
{
    ...
    msgQ.type = SIR_HAL_WOWL_EXIT_REQ;
    msgQ.reserved = 0;
    msgQ.bodyptr = 0;
    msgQ.bodyval = 0;

    pmmLog(pMac, LOGW, FL("Sending SIR_HAL_WOWL_EXIT_REQ"));

    /* we need to defer any incoming messages until
     * we get a SIR_HAL_WOWL_EXIT_RSP from HAL.
     */
    SET_LIM_PROCESS_DEFD_MESGS(pMac, false);

    if( eSIR_SUCCESS != (retCode = halPostMsgApi( pMac, &msgQ )))
        pmmLog( pMac, LOGE,
            FL("Posting SIR_HAL_WOWL_EXIT_REQ failed, reason=%X\n"),
            retCode );
...
\brief   This function process the SIR_HAL_WOWL_EXIT_RSP message.
...
{
    /* we need to process all the deferred messages enqueued
     * since the initiating the SIR_HAL_WOWL_EXIT_REQ.
     */
    SET_LIM_PROCESS_DEFD_MESGS(pMac, true);
    ...
    if(rspStatus == eHAL_STATUS_SUCCESS)
    {
        ...
        limSendSmeRsp(pMac, eWNI_PMC_EXIT_WOWL_RSP, eSIR_SME_SUCCESS);
    }
    else
    {
        pmmLog(pMac, LOGE, FL("Rcvd failure rsp from HAL to exit WOWLAN \n"));
        limSendSmeRsp(pMac, eWNI_PMC_EXIT_WOWL_RSP, eSIR_SME_WOWL_EXIT_REQ_FAILED);
    }
    return;
}
...
tSirRetStatus pmmImpsSendChangePwrSaveMsg(tpAniSirGlobal pMac, tANI_U8 mode)
{
    ...
    {
        msgQ.type = SIR_HAL_ENTER_IMPS_REQ;
        PELOGW(pmmLog (pMac, LOGW, FL("Sending SIR_HAL_ENTER_IMPS_REQ to HAL\n"));)
    }
    else
    {
        msgQ.type = SIR_HAL_EXIT_IMPS_REQ;
        PELOGW(pmmLog (pMac, LOGW, FL("Sending SIR_HAL_EXIT_IMPS_REQ to HAL\n"));)
    }
    ...
    /* we need to defer any incoming messages until we get a
     * SIR_HAL_ENTER_IMPS_REQ or SIR_HAL_EXIT_IMPS_RSP from HAL.
     */
    SET_LIM_PROCESS_DEFD_MESGS(pMac, false);
    MTRACE(macTraceMsgTx(pMac, 0, msgQ.type));
    retStatus = halPostMsgApi(pMac, &msgQ);
    ...
    {
        PELOGE(pmmLog(pMac, LOGE, 
            FL("SIR_HAL_ENTER/EXIT_IMPS_REQ to HAL failed, reason=%X\n"), retStatus);)
    }
 ...
 *
 * FUNCTION:
 * This function is called to send either SIR_HAL_ENTER_UAPSD_REQ
 * or SIR_HAL_EXIT_UAPSD_REQ to HAL.
 *
 * NOTE:
 *
...
tSirRetStatus pmmUapsdSendChangePwrSaveMsg (tpAniSirGlobal pMac, tANI_U8 mode)
{
    ...
    if (SIR_PM_SLEEP_MODE == mode)
    {
        ...
        msgQ.type = SIR_HAL_ENTER_UAPSD_REQ;
        ...
        PELOGW(pmmLog (pMac, LOGW, FL("pmmUapsd: Sending SIR_HAL_ENTER_UAPSD_REQ to HAL\n"));)
    }
    else
    {
        msgQ.type = SIR_HAL_EXIT_UAPSD_REQ;
        msgQ.bodyptr = NULL;
        PELOGW(pmmLog (pMac, LOGW, FL("pmmUapsd: Sending SIR_HAL_EXIT_UAPSD_REQ to HAL\n"));)
    }

    /* we need to defer any incoming messages until we get a
     * SIR_HAL_ENTER/EXIT_UAPSD_RSP from HAL.
     */
    SET_LIM_PROCESS_DEFD_MESGS(pMac, false);
    ...
    {
        PELOGE(pmmLog(pMac, LOGE, 
            FL("pmmUapsd: SIR_HAL_ENTER/EXIT_UAPSD_REQ to HAL failed, reason=%X\n"),
            retStatus);)
        if (SIR_PM_SLEEP_MODE == mode)
...
/*
	...
	{
		__do_div64( TimeAwake , (pMac->pmm.ImpsSleepTimeStamp - pMac->pmm.ImpsWakeupTimeStamp) , 10 );
	}
	else
	{
		TimeAwake = 0;
	...
	__do_div64( pMac->pmm.ImpsAvgTimeAwake , ((pMac->pmm.ImpsAvgTimeAwake * pMac->pmm.ImpsCntSleep) + TimeAwake) ,(pMac->pmm.ImpsCntSleep + 1));
...
/*
	...
	__do_div64( SleepTime , (pMac->pmm.ImpsWakeupTimeStamp - pMac->pmm.ImpsSleepTimeStamp) , 10);
	...
	__do_div64( pMac->pmm.ImpsAvgSleepTime ,  ( (pMac->pmm.ImpsAvgSleepTime * pMac->pmm.ImpsCntAwake) + SleepTime) , (pMac->pmm.ImpsCntAwake + 1));
