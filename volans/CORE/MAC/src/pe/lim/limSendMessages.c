 ...
 *
 *FUNCTION:
 * This function is called to send CFP Parameters to HAL, when they are changed.
 *
 *LOGIC:
 *
 *ASSUMPTIONS:
...
tSirRetStatus limSendCFParams(tpAniSirGlobal pMac, tANI_U8 bssIdx, tANI_U8 cfpCount, tANI_U8 cfpPeriod)
{
    ...
    msgQ.type = SIR_HAL_UPDATE_CF_IND;
    msgQ.reserved = 0;
    msgQ.bodyptr = pCFParams;
    msgQ.bodyval = 0;

    limLog( pMac, LOG3,
                FL( "Sending SIR_HAL_UPDATE_CF_IND..." ));
 ...
 * parameters to HAL, which are changed and indication is received in beacon.
 *
 *LOGIC:
 *
 *ASSUMPTIONS:
								...
								tpPESession  psessionEntry )
{
    ...
    msgQ.type = SIR_HAL_UPDATE_BEACON_IND;
    msgQ.reserved = 0;
    msgQ.bodyptr = pBcnParams;
    msgQ.bodyval = 0;

    limLog( pMac, LOG3,
                FL( "Sending SIR_HAL_UPDATE_BEACON_IND, paramChangeBitmap in hex = %x" ),
 ...
 *
 *FUNCTION:
 * This function is called to send Channel Switch Indication to HAL
 *
 *LOGIC:
 *
 *ASSUMPTIONS:
...
#endif
{
    ...
    msgQ.type = SIR_HAL_CHNL_SWITCH_REQ;
    msgQ.reserved = 0;
    ...
    if( eSIR_SUCCESS != (retCode = halPostMsgApi( pMac, &msgQ )))
    {
        ...
        limLog( pMac, LOGP,
                    FL("Posting  SIR_HAL_CHNL_SWITCH_REQ to HAL failed, reason=%X\n"),
                    retCode );
    }
 ...
 *
 *FUNCTION:
 * This function is called to send dynamically changing EDCA Parameters to HAL.
 *
 *LOGIC:
 *
 *ASSUMPTIONS:
...
tSirRetStatus limSendEdcaParams(tpAniSirGlobal pMac, tSirMacEdcaParamRecord *pUpdatedEdcaParams, tANI_U16 bssIdx, tANI_BOOLEAN highPerformance)
{
    ...
    msgQ.type = SIR_HAL_UPDATE_EDCA_PROFILE_IND;
    msgQ.reserved = 0;
    ...
    if( eSIR_SUCCESS != (retCode = halPostMsgApi( pMac, &msgQ )))
    {
        ...
        limLog( pMac, LOGP,
                    FL("Posting  SIR_HAL_UPDATE_EDCA_PROFILE_IND to HAL failed, reason=%X\n"),
                    retCode );
    }
...
/** ---------------------------------------------------------
\fn      limSetLinkState
\brief   LIM sends a message to HAL to set the link state
...
tSirRetStatus limSetLinkState(tpAniSirGlobal pMac, tSirLinkState state,tSirMacAddr bssId)
{
    ...
    retCode = (tANI_U32)halPostMsgApi(pMac, &msgQ);
    if (retCode != eSIR_SUCCESS)
    {
...
\brief   LIM sends a SIR_HAL_SET_TX_POWER_REQ message to HAL 
\param   tpAniSirGlobal      pMac
...
tSirRetStatus limSendSetTxPowerReq(tpAniSirGlobal pMac,  tpSirSetTxPowerReq pTxPowerReq)
{
    ...
    msgQ.type = SIR_HAL_SET_TX_POWER_REQ;
    msgQ.reserved = 0;
    msgQ.bodyptr = pTxPowerReq;
    msgQ.bodyval = 0;

    PELOGW(limLog(pMac, LOGW, FL( "Sending SIR_HAL_SET_TX_POWER_REQ to HAL"));)

    MTRACE(macTraceMsgTx(pMac, 0, msgQ.type));
    if( eSIR_SUCCESS != (retCode = halPostMsgApi( pMac, &msgQ )))
    {
        limLog( pMac, LOGP, FL("Posting SIR_HAL_SET_TX_POWER_REQ to HAL failed, reason=%X"), retCode );
        if (NULL != pTxPowerReq)
        {
...
\brief   LIM sends a SIR_HAL_GET_TX_POWER_REQ message to HAL
\param   tpAniSirGlobal      pMac
...
tSirRetStatus limSendGetTxPowerReq(tpAniSirGlobal pMac,  tpSirGetTxPowerReq pTxPowerReq)
{
    ...
    msgQ.type = SIR_HAL_GET_TX_POWER_REQ;
    msgQ.reserved = 0;
    msgQ.bodyptr = pTxPowerReq;
    msgQ.bodyval = 0;

    PELOGW(limLog(pMac, LOGW, FL( "Sending SIR_HAL_GET_TX_POWER_REQ to HAL"));)

    MTRACE(macTraceMsgTx(pMac, 0, msgQ.type));
    if( eSIR_SUCCESS != (retCode = halPostMsgApi( pMac, &msgQ )))
    {
        limLog( pMac, LOGP, FL("Posting SIR_HAL_GET_TX_POWER_REQ to HAL failed, reason=%X"), retCode );
        if (NULL != pTxPowerReq)
        {
            palFreeMemory( pMac->hHdd, (tANI_U8 *) pTxPowerReq);
        }
...
\brief   LIM sends beacon filtering info to HAL
\param   tpAniSirGlobal  pMac
\return  None
  -----------------------------------------------------------*/
tSirRetStatus limSendBeaconFilterInfo(tpAniSirGlobal pMac)
{
    ...
    msgQ.type = SIR_HAL_BEACON_FILTER_IND;
    msgQ.reserved = 0;
    msgQ.bodyptr = pBeaconFilterMsg;
    msgQ.bodyval = 0;

    limLog( pMac, LOG3, FL( "Sending SIR_HAL_BEACON_FILTER_IND..." ));
    MTRACE(macTraceMsgTx(pMac, 0, msgQ.type));
    if( eSIR_SUCCESS != (retCode = halPostMsgApi( pMac, &msgQ )))
    {
