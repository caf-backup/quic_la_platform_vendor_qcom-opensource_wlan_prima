...
__limProcessAddTsReq(tpAniSirGlobal pMac, tANI_U8 *pBd,tpPESession psessionEntry)
{
    ...
    {
        if ((! pMac->lim.gLimWmeEnabled) || (! pSta->wmeEnabled))
        {
    ...
    {
        if ((! pMac->lim.gLimWsmEnabled) || (! pSta->wsmEnabled))
        {
    ...
    else if ((! pMac->lim.gLimQosEnabled) || (! pSta->lleEnabled))
    {
...
__limProcessDelTsReq(tpAniSirGlobal pMac, tANI_U8 *pBd,tpPESession psessionEntry)
{
    ...
    else if ((pMac->lim.gLimWsmEnabled) && pSta->wsmEnabled)
        {
...
static void
__limProcessMeasurementRequestFrame(tpAniSirGlobal pMac, tANI_U32 *pBd)
{
    tpSirMacMgmtHdr                       pHdr;
    tANI_U8                                    *pBody;
    tpSirMacMeasReqActionFrame            pMeasReqFrame;
...
static void
__limProcessTpcRequestFrame(tpAniSirGlobal pMac, tANI_U32 *pBd)
{
    tpSirMacMgmtHdr                       pHdr;
    tANI_U8                                    *pBody;
...
tSirMacStatusCodes statusCode = eSIR_MAC_STA_BLK_ACK_NOT_SUPPORTED_STATUS;
  ...
  // Validate if a BA is active for the requested TID
        ...
        pSta->tcCfg[baParameterSet.tid].fUseBARx )
    {
      ...
      limLog( pMac, LOGW,
          FL("Valid DELBA Ind received. Time to send SIR_HAL_DELBA_IND to HAL...\n"));
    }
    else
      limLog( pMac, LOGW,
 ...
 * \param pMac The global tpAniSirGlobal object
 *
 * \param pBd Handle to the Rx BD from HDD
 *
 * \return none
 *
 */
...
tANI_U8 *pBody;
  ...
  //
  // Post SIR_HAL_DELBA_IND to HAL and delete the
  // existing BA session
  //
        ...
        case SIR_MAC_ACTION_SPECTRUM_MGMT:
            switch (pActionHdr->actionID)
            {
                ...
                case SIR_MAC_ACTION_MEASURE_REQUEST_ID:
                    if(pMac->lim.gLim11hEnable)
                    {
                        __limProcessMeasurementRequestFrame(pMac, pBd);
                    }
                    break;
                ...
                case SIR_MAC_ACTION_TPC_REQUEST_ID:
                        ...
                        (pMac->lim.gLimSystemRole == eLIM_AP_ROLE))
                    {
                        if(pMac->lim.gLim11hEnable)
                        {
                            __limProcessTpcRequestFrame(pMac, pBd);
                        }
                    }
                    break;
