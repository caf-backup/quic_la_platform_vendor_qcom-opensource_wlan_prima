...
tANI_U32 val = 0;
  ...
  msgQ.type = SIR_HAL_SET_BSSKEY_REQ;
  //
  // FIXME_GEN4
  ...
  limLog( pMac, LOGW,
      FL( "Sending SIR_HAL_SET_BSSKEY_REQ...\n" ));
  ...
  if( eSIR_SUCCESS != (retCode = halPostMsgApi( pMac, &msgQ )))
  {
    ...
    return; // Continue after SIR_HAL_SET_BSSKEY_RSP...
...
tANI_U32 val = 0;
  ...
  msgQ.bodyval = 0;
    ...
    limLog( pMac, LOG1, FL( "Sending SIR_HAL_SET_STAKEY_REQ...\n" ));
    MTRACE(macTraceMsgTx(pMac, 0, msgQ.type));
    if( eSIR_SUCCESS != (retCode = halPostMsgApi( pMac, &msgQ ))) {
    ...
    return; // Continue after SIR_HAL_SET_STAKEY_RSP...
...
tSirRetStatus      retCode;
  ...
  limLog( pMac, LOGW,
      FL( "Sending SIR_HAL_REMOVE_BSSKEY_REQ...\n" ));
  MTRACE(macTraceMsgTx(pMac, 0, msgQ.type));
...
tSirRetStatus      retCode;
  ...
  // Update the SIR_HAL_REMOVEKEY_REQ parameters
  ...
  msgQ.type = SIR_HAL_REMOVE_STAKEY_REQ;
  //
  // FIXME_GEN4
  // A global counter (dialog token) is required to keep track of
  // all PE <-> HAL communication(s)
  ...
  limLog( pMac, LOGW,
      FL( "Sending SIR_HAL_REMOVE_STAKEY_REQ...\n" ));
  MTRACE(macTraceMsgTx(pMac, 0, msgQ.type));
  if( eSIR_SUCCESS != (retCode = halPostMsgApi( pMac, &msgQ )))
  {
    limLog( pMac, LOGE,
