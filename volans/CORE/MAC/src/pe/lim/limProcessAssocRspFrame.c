...
void
limProcessAssocRspFrame(tpAniSirGlobal pMac, tANI_U32 *pBd, tANI_U8 subType,tpPESession psessionEntry)
{
    tANI_U8               *pBody;
    tANI_U16              caps;
    tANI_U32              frameLen;
...
#ifdef FEATURE_WLAN_CCX
        {
            ...
            if (pAssocRsp->tsmPresent)
            {
                ...
                if (pAssocRsp->tspecPresent) {
                    ...
                    limLog(pMac, LOGW, "TSM present but TSPEC IE not present in Reassoc Rsp\n");
                }
            }
        }
#endif
