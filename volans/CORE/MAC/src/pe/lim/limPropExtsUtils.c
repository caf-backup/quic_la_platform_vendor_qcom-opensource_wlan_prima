 ...
 *
 * @param  pMac     - Pointer to Global MAC structure
 * @param  pBd      - Pointer to received BD+payload
...
limCollectMeasurementData(tpAniSirGlobal pMac,
                          tANI_U32 *pBd, tpSchBeaconStruct pBeacon)
{
   ...
   PELOG3(limLog(pMac, LOG3, FL("Collecting measurement data for RadioId %d\n"),
	...
	if (chanId == 0)
		chanId = SIR_MAC_BD_TO_RX_CHANNEL(pBd);
    ...
    pNewMatrix->matrix.aggrRssi += SIR_MAC_BD_TO_RSSI_DB(pBd);
    ...
    pNode->info.neighborBssInfo.rssi = SIR_MAC_BD_TO_RSSI_DB(pBd);
